...
ThisCodex는 셸 의존이 없는 Node 설치기를 동봉합니다.
...
`--check`는 파일을 쓰지 않습니다. `--apply`는 `thiscodex` 스킬을 Codex가
스캔하는 계층(기본 `~/.agents/skills/thiscodex`)으로 복사하고, 선택 시
`~/.codex/config.toml`을 백업 후 패치하며, OS별 runner 실행 안내를
출력합니다. scope A에서는 데몬을 자동 시작하지 않습니다.

ThisCodex 설치는 매니페스트 기반입니다. 설치기는
`install/thiscodex.install.json`을 읽고, 각 단계는 순서·이유·안전 라벨·검증
조건을 가집니다. `thiscodex doctor`는 같은 verify 검증을 다시 실행하므로
