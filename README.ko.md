...
| 디스코드 스레드 만들기 | ✅ **ThisCodex CLI** | 공개·비공개 스레드 만들기는 Discord 공식 기능이다. 서버·봇 권한에서 `CREATE_PUBLIC_THREADS`(「공개 스레드 만들기」)와 `CREATE_PRIVATE_THREADS`(「비공개 스레드 만들기」)를 허용한다. ThisCodex는 Discord 공식 REST API를 직접 호출한다. 현재 동봉된 공식 Discord MCP에는 이 생성 명령만 노출되지 않아, ThisCodex가 검토 우선 CLI로 제공한다. 공개 스레드: `thiscodex discord-thread public --channel-id <id> --channel-type <0\|5> --message-id <id> --name "공개 스레드"`; 비공개 스레드: `thiscodex discord-thread private --channel-id <id> --channel-type 0 --name "비공개 스레드"`. 먼저 계획만 확인하고 `--apply`를 붙여 다시 실행한다. `--channel-type`은 조회값이 아니라 운영자 선언값이다. 실행 전 Discord 채널 객체의 `type`(0=일반 텍스트, 5=공지)을 확인하며, 불일치는 Discord가 판정한다. 비공개 모드는 `invitable=false`가 기본이고, 참여자의 추가 초대를 허용할 때만 `--invitable true`를 명시한다. `reply_to`는 생성이 아니라 답장 참조다. |
...
봇 *user* 가 아니라 동명 *역할* `<@&…>` 을 부른 경우 — 초대 시 같은 이름 역할이
자동 생성된다), **🔍 는 붙었는데 답이 없으면 아래 나가는 쪽 문제.** 순서대로
확인 — 브리지 로그(`run.sh` 의 `infra` 창)가 각 항목을 경고해 준다:

| 확인 | 무엇이 깨졌나 | 처방 |
|---|---|---|
| `~/.codex/config.toml` 에 `[mcp_servers.discord]` 있나? | codex 에 답장 도구 자체가 없음(새 기기) | §3.2 블록 추가 |
| 그 항목의 `DISCORD_STATE_DIR` = **이** 봇인가? | 답장이 다른 봇 명의로 나가거나 그쪽 토큰에서 죽음 | 생성된 `infra-launch.sh` 가 `-c` 로 고정해 줌; 수동 실행은 config.toml 정렬 |
| `.env` 줄바꿈이 LF 인가? (윈도우 붙여넣기 = CRLF) | bot.py 는 로그인까지 되는데 디스코드 도구만 «토큰 없음»으로 죽음 | `sed -i 's/\r$//' <상태폴더>/.env` |
| `BOT_WD/AGENTS.md` 있나? (§3.3) | 모델이 답을 쓰고도 **도구를 안 부름** — 말 없는 봇, 오류 0 | `init --apply` 가 이제 만들어 줌; Discord Reply Rule 절 유지 |
| `<상태폴더>/access.json` 에 내 채널이 있나? | MCP 는 뜨는데 **발신을 전부 거부**(`reply failed: … not allowlisted`) — 듣긴 하는데 답만 안 옴; 이제 브리지가 거부 사유를 채널에 알려 줌 | 씨앗으로 깔린 `access.json.example` 복사 → 채널·유저 id 기입 → 재시작 |

존재 ≠ 행동: 고친 뒤에는 **고친 시점 이후의 실제 왕복 1회**로 증명한다
(수리 전에 온 답장은 아무것도 증명하지 않는다).
