...
4. **Reply routing.** A configured channel (`SLACK_CHANNEL_ID`) accepts an explicit `chat_id` from the first send. A DM or any other conversation is only a valid target after an inbound from it has been seen; until then the bridge falls back to the home channel. To open such a conversation first, post via the Web API (`chat.postMessage`) directly.
