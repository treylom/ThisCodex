...
> **Implementation note**: `init`, `doctor`, `smoke`, and `discord-thread` are real CLI
...
| Create a Discord thread | Creating public and private threads is an official Discord feature; enable `CREATE_PUBLIC_THREADS` (「공개 스레드 만들기」) and `CREATE_PRIVATE_THREADS` (「비공개 스레드 만들기」) in the server/bot permissions. ThisCodex calls Discord's official REST API directly. Only the matching command is absent from the currently shipped official Discord MCP, so use `thiscodex discord-thread <public\|private> ...`: it emits a non-network plan first; inspect it, then repeat with `--apply`. `--channel-type` is an operator-declared value, not a lookup: verify the Discord channel object's `type` (0=text, 5=announcement) before applying; Discord decides any mismatch. Private mode defaults `invitable` to `false`; add `--invitable true` only when members may add others. |
| Check feature availability | `/thiscodex features` — lists what's working (Discord bot, reactions via `mcp__discord__react`, thread history via `mcp__discord__fetch_messages`, multi-client same-thread, YOLO, image_gen, web.run) and separates official Discord features from the commands exposed by the official Discord MCP. ThisCodex directly calls the official Discord REST API to create a 공개 스레드 (public thread) or 비공개 스레드 (private thread) with the dry-run-first `thiscodex discord-thread` CLI; `reply_to` is not thread creation. |
