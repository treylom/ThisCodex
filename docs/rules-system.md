...
Where a rule maps to a **detectable action** (a tool call, a Stop, an outbound deliverable), a Stop or PreToolUse hook checks for the violation signal and blocks with a one-line reminder (Stop blocks via `{"decision":"block","reason":...}`). Shipped examples: `completion-gate.sh` (pre-report before declaring "done" to the user), `dispatch-verify.sh` (verify a delegated bot actually started before ending the turn), `reply-gate.sh` (deliver via the channel tool, not terminal-only). `copy-coherence-gate.sh` is the routing (non-blocking) variant: dual-anchored on PostToolUse(Write|Edit|MultiEdit) + PreToolUse(Bash `git commit`), it detects fact-document changes and injects a copy-sweep instruction. Two delivery contracts matter here, both measured: a Stop hook blocks only via `{"decision":"block","reason":...}` JSON, and **a hook's plain stdout is NOT injected into the model context** (measured on UserPromptSubmit; the `{"hookSpecificOutput":{"hookEventName":...,"additionalContext":...}}` JSON path is measured **delivered** on PostToolUse — positive and negative probes) — emit the JSON envelope or the model never sees it. Verify hooks at three layers: registered → fired (log line) → **delivered** (the agent's behavior changes); the first two without the third is a dead gate wearing a green light.
