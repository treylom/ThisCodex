...
| Discord reaction / existing-thread history | Official Discord MCP `react` / `fetch_messages` | Call `mcp__discord__react(chat_id,message_id,emoji)` or `mcp__discord__fetch_messages(channel=<thread_id>)` only when those tools are actually exposed. Thread access inherits the parent-channel allowlist and Discord permissions. |
| Discord thread creation | **ThisCodex-owned `discord-thread` CLI** calling the official Discord REST API | Creating public and private threads is an official Discord feature; enable `CREATE_PUBLIC_THREADS` (「공개 스레드 만들기」) and `CREATE_PRIVATE_THREADS` (「비공개 스레드 만들기」) in the server/bot permissions. ThisCodex calls that official feature directly. Only the matching command is absent from the currently shipped official Discord MCP, so the CLI provides it. Public thread (공개 스레드): `thiscodex discord-thread public --channel-id <id> --channel-type <0\|5> --message-id <id> --name <name>`. Private thread (비공개 스레드): `thiscodex discord-thread private --channel-id <id> --channel-type 0 --name <name>`. Both default to a non-network dry-run; inspect the plan before repeating with `--apply`. `--channel-type` is an operator-declared value, not a lookup: verify the Discord channel object's `type` (0=text, 5=announcement) before applying; Discord decides any mismatch. Private mode defaults `invitable` to `false`; add `--invitable true` only when members may add others. `reply_to` remains a reply reference, not creation. |
...
| Discord 반응 / 기존 스레드 기록 읽기 | 공식 Discord MCP `react` / `fetch_messages` | 실제 도구 노출을 먼저 확인한 뒤 `mcp__discord__react(chat_id,message_id,emoji)` 또는 `mcp__discord__fetch_messages(channel=<thread_id>)`를 호출한다. 스레드 접근은 부모 채널 allowlist와 Discord 권한을 승계한다. |
| Discord 스레드 만들기 | **Discord 공식 REST API를 호출하는 ThisCodex 소유 `discord-thread` CLI** | 공개·비공개 스레드 만들기는 Discord 공식 기능이다. 서버·봇 권한에서 `CREATE_PUBLIC_THREADS`(「공개 스레드 만들기」)와 `CREATE_PRIVATE_THREADS`(「비공개 스레드 만들기」)를 허용한다. ThisCodex는 이 공식 기능을 직접 호출한다. 현재 동봉된 공식 Discord MCP에는 이 생성 명령만 노출되지 않아 CLI로 제공한다. 공개 스레드: `thiscodex discord-thread public --channel-id <id> --channel-type <0\|5> --message-id <id> --name <name>`. 비공개 스레드: `thiscodex discord-thread private --channel-id <id> --channel-type 0 --name <name>`. 둘 다 기본값은 네트워크를 쓰지 않는 계획 출력이며, 확인 뒤 `--apply`로 실행한다. `--channel-type`은 조회값이 아니라 운영자 선언값이다. 실행 전 Discord 채널 객체의 `type`(0=일반 텍스트, 5=공지)을 확인하며, 불일치는 Discord가 판정한다. 비공개 모드는 `invitable=false`가 기본이고, 참여자의 추가 초대를 허용할 때만 `--invitable true`를 명시한다. `reply_to`는 생성이 아니라 답장 참조다. |
