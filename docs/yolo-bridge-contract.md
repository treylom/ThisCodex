...
- **Bridge layer**: `examples/bot.py` emits a heartbeat through the same
  Discord reply/send path used by the bridge while a turn runs, cancelled the
  instant the turn finishes. The interval is configured by
  `THISCODEX_HEARTBEAT_SEC`; setup maps `progress_report_cadence` as
  `1m→60`, `3m→180`, `5m→300`, and `off→0`. `per_task` / "작업마다" does not
  create a new timer; it means the existing completion / partial / blocked
  reporting path must fire on meaningful subtasks or milestones, not raw model
  turn boundaries. The heartbeat is generic ("still working, N min elapsed") —
  it must never leak file paths, tool output, or other sensitive content.
...
- **bridge 계층**: `examples/bot.py` 가 기존 bridge Discord reply/send 경로로
  ...
  `THISCODEX_HEARTBEAT_SEC` 로 설정하며 setup 의 `progress_report_cadence`
  는 `1m→60`, `3m→180`, `5m→300`, `off→0` 으로 매핑한다. `per_task` /
  "작업마다"는 새 타이머가 아니라 기존 완료 / 부분 / blocked 보고 경로를
  유의미한 subtask·마일스톤 완료에 연결한다는 뜻(raw turn 경계 아님).
  heartbeat 는 generic("작업 중, N분 경과") — 경로·도구출력·민감내용 유출 금지.
